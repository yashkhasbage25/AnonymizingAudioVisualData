...
def parse_args():
    ...
    parser.add_argument('--audio_only', action='store_true', help='specify this option if the input is audio-only')
...
if __name__ == '__main__':
    ...
    if not args.audio_only:
        ...
        audio_py_path = osp.join(osp.dirname(__file__), 'audio.py')
        assert osp.exists(audio_py_path), f"file not found: {audio_py_path}"
        cmd = f"python {audio_py_path} --inpath {args.inpath} --outpath {args.outpath} --tr pitch --pitch_n_semitones {args.pitch}"
        error = os.system(
            cmd
        )
        if error:
            raise Exception(f"unable to change audio. Check audio.py. error code: {error}")
        print('audio saved at', args.outpath)